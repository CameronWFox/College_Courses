...
{ pqueue_t master_queue;	// For this simulation, I'm going to maintain ONE event queue.  This is it

  RNG *rng_pointer;		// For this simulation, I'm going to maintain ONE core random number generator.
				// All pseudo-random numbers will be derived from this core generator.
				
	
  int    number_of_lines_open = 10;  // The number of lines that are open	
  double master_clock = 0.0;        // Master World Clock
  
  int    c = 0;                     // Generic counter
  double arrive_at_park_time;       // Time at which someone arrived at the park. 
  double q_enter_time;              // Temp holder for enter time popped from queue
  double q_exit_time;               // Temp holder for exit time popped from queue
  double q_wait_time;               // Temp holder for how long someone needs to wait
  int    q_identity_code;           // Temp holder for ID code popped from queue
  int    q_group_code;              // Temp holder for group code popped from queue

  int    guests_served = 0;         // World state variable
  double total_wait_time = 0.0;     // World state variable

  int    line_count;
  int    shortest_line;
  int    temp_shortest_line_length;
  int    shortest_line_length;
  
  pqueue_init(&master_queue, "Master Queue"); // Allocate memory and initialize event queue to zero events

  rng_pointer = rng_create(); // Create and seed a PSEUDO random number generator that does NOT use 
                              // any system or kernel provided random number devices.


  if (argc > 1) sscanf(argv[1], "%d", &number_of_lines_open);

  // Fill the queue with initial events

  
  if (argc > 2)
     while (!feof(stdin))
       { c++;
         fscanf(stdin,"%lf", &arrive_at_park_time);
         pqueue_push(&master_queue, arrive_at_park_time, arrive_at_park_time, c, 0);
       }
       
  if (argc < 2)
    for (c = 0; c < 30000; c++)
      { arrive_at_park_time = rng_uniform(rng_pointer, 10.0, 22.0); // Select a uniform random time from 
								                                    // hour 10 to hour 22 of the day

        // Put the arrival of a guest on the event queue.  We're going to use the following conventions:
        //
	    // pqueue_push(pqueue_t *queue, double enter_time, double exit_time, int identity_code, int group_code)
        // 
        // enter_time = arrive_at_park_time   time guest arrived at the turnstile
        // exit_time  = arrive_at_park_time   the event of "arriving" is "zero duration".  exit_time == enter_time
        // idenity_code = c                   let's give each guest a unique ID based on what order she/he hit the turnstile
        // group_code   = 1                   we'll use the group code to talk about the type of event.  In this case, '1'
        //                                    means it's an "arrive at park code" 
  ...
  // At this point, our event queue is filled with NOTHING except arrival events "waiting" to happen.
  // Let's have a look at the queue.
  
  pqueue_print(&master_queue, 0);
  
  // Now, let's begin a processing loop! :)
  // We'll pull events out of the queue, in order of EXIT TIME.  As we pull
  // each one off, we'll process it.  Some events might trigger changes to
  // world state.  Some might trigger new events that need to go back into
  // the queue.  Some might do both.
  
  while (!pqueue_empty(&master_queue))
    { // Get "soonest" event from queue
       pqueue_pop(&master_queue, &q_enter_time, &q_exit_time, &q_identity_code, &q_group_code);
    
      // If the event is a "arrive at park event".  Make a NEW event that models when
      // that person gets OUT of line.  Put it in the queue.  Note that the code for
      // "person in line" is an integer from 1 to the number of turnstiles (lines)
      // that are open.
      
      if (q_group_code == 0)
         {  // Which line should this person go to ?
            shortest_line = 1;
            shortest_line_length = pqueue_size_group(&master_queue, shortest_line);
            for (line_count = 2; line_count <= number_of_lines_open; line_count++)
                { temp_shortest_line_length = pqueue_size_group(&master_queue, line_count);
                  if (temp_shortest_line_length < shortest_line_length)
                     { shortest_line_length = temp_shortest_line_length;
                       shortest_line = line_count;
                     }
                }
                                
            // how long does this person have to wait?
            // Five seconds per person in line plus five seconds for oneself.
            
            q_wait_time = (5.0/3600.0) + (5.0/3600.0) * (double) pqueue_size_group(&master_queue, shortest_line);
            pqueue_push(&master_queue, q_enter_time, q_enter_time + q_wait_time, 
                        q_identity_code, shortest_line);
                                                
            printf("Guest %6d ENTERED LINE %d at %5.2lf and will wait %lf hours\n", q_identity_code, shortest_line, q_enter_time, q_wait_time);
            
         }
         
      // What if we have a "get out of line event?"

      if (q_group_code >= 1)
        { printf("Guest %6d EXITED  LINE %d at %5.2lf and waited    %lf hours\n", q_identity_code, q_group_code, q_exit_time, q_exit_time - q_enter_time);
          
          // Let's update world state ;)
          
          guests_served++;
          total_wait_time += q_exit_time - q_enter_time;
        
        }
        
      // Do generic processing that's true for any event
      
      master_clock = q_exit_time;   // Update master clock
      
      // Uncomment the next code line to implement the idea that the number of open gates is
      // set to 20 at or after 4:00 PM.  Use similar rules to implement other changes
      // of staffing
      
      //if (master_clock > 16.0)  number_of_lines_open = 20;  // If it's after 4:00 PM, have 20 lines open
         


    }
  
    // Print out final world state statistics
    
    printf("\n");
    
    printf("%05d guests were served with an average wait time of %lf hours == %lf minutes == %lf seconds)\n", guests_served, total_wait_time / (double)guests_served, 
                                                                                                              60.0 * (total_wait_time / (double)guests_served),
                                                                                                              3600.0 * (total_wait_time / (double)guests_served));
