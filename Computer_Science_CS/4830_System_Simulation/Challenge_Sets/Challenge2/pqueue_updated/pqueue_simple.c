...
{ pqueue_t master_queue;	// For this simulation, I'm going to maintain ONE event queue.  This is it
  ...
  for (c = 0; c < 100; c++)
	...
	// pqueue_push(pqueue_t *queue, double enter_time, double exit_time, int identity_code, int group_code)
        ...
        pqueue_push(&master_queue, arrive_at_park_time, arrive_at_park_time, c, 1);
      }
  ...
  while (!pqueue_empty(&master_queue))
    { // Get "soonest" event from queue
      ...
      // "person in line" is the integer 2
      
      if (q_group_code == 1)
         { // how long does this person have to wait?
            q_wait_time = (600.0/3600.0) + (600.0/3600.0) * (double) pqueue_size_group(&master_queue, 2);
            pqueue_push(&master_queue, q_enter_time, q_enter_time + q_wait_time, 
                        q_identity_code, 2);
                        
            printf("Guest %4d ENTERED the line at %5.2lf and will have to wait %lf hours\n", q_identity_code, q_enter_time, q_wait_time);
      ...
      if (q_group_code == 2)
        { printf("Guest %4d EXITED the line at %5.2lf and waited %lf hours\n", q_identity_code, q_exit_time, q_exit_time - q_enter_time);
    ...
    printf("%05d guests were served with an average wait time of %lf hours\n", guests_served, total_wait_time / (double)guests_served);
